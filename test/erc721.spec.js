  ...
  //  const tokenUri1 = "This is data for the token 1"; // Does not have to be unique
   ...
   // const tokenUri2 = "This is data for the token 2"; // Does not have to be unique
    ...
    it(' should be able to deploy and mint ERC721 token', async () => {
        token = await YYToken.new(name, symbol)
     //   await token.mintOne(account1, tokenId1, tokenUri1, {from: accounts[0], gas:5000000000})
        await token.mintOne(account1, tokenId1)//, {from: accounts[0], gas:5000000000})
    ...
    it(' should be unique', async () => {
      //  const duplicateTokenID = token.mintOne(account2, tokenId1, tokenUri2, {from: accounts[0], gas:5000000000}) //tokenId
        const duplicateTokenID = token.mintOne(account2, tokenId1)// {from: accounts[0], gas:5000000000}) //tokenId
    ...
    it(' should allow creation of multiple unique tokens and manage ownership', async () => {
      //  const additionalToken = await token.mintOne(account2, tokenId2, tokenUri2, {from: accounts[0], gas:5000000000})
        const additionalToken = await token.mintOne(account2, tokenId2)//, {from: accounts[0], gas:5000000000})
